...
namespace AmazonExcelAddIn.UserLibrary
{
    public class MySQLHelper
    {
        ...
        /// 初始化MySqlHelper实例
        /// </summary>
        /// <param name="Host">数据库连接地址</param>
        /// <param name="Username">数据库用户</param>
        /// <param name="Password">数据库密码</param>
        /// <param name="Database">数据库名称</param>
        /// <param name="Default">是否选择默认数据库连接</param>
        public MySQLHelper(bool Default = false, string Host = "", string Database = "", string Username = "", string Password = "")
        {
            if (Default)
            {
                Host = VariableHelper.MysqlHost;
                Database = VariableHelper.MysqlDatabase;
                Username = VariableHelper.MysqlUserName;
                Password = VariableHelper.MysqlPassWord;
            }
            this.ConnectionString = $"Host={Host};Database={Database};Username={Username};Password={Password}";
        }
        ...
        {
            LoggerHelper.Debug($"变量 commandText > {commandText}");
        ...
        {
            LoggerHelper.Debug($"commandType >{commandType} commandText > {commandText}");
        ...
        {
            LoggerHelper.Debug($"commandText > {commandText}");
        ...
        {
            LoggerHelper.Debug($"connectionString >{connectionString} commandText > {commandText}");
        ...
        {
            LoggerHelper.Debug($"connectionString >{connectionString} commandType > {commandType} commandText > {commandText}");
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
        ...
        {
            LoggerHelper.Debug($"commandType > {commandType} commandType > {commandType} commandText > {commandText}");
            MySqlCommand command = new MySqlCommand();
            PrepareCommand(command, connection, transaction, commandType, commandText, parms);
        ...
        {
            LoggerHelper.Debug($"commconnectionStringandType > {connectionString} commandText > {commandText}");
        ...
        {
            LoggerHelper.Debug($"connectionString > {connectionString} tableName > {tableName}");
